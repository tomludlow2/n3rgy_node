...
const fs = require('fs');
const electricity_to_cost = require("./functions/electricity_to_cost.js");
...
const req_e = https.request(options_electricity, (res) => {
	...
	res.on('end', () => {
		try {
			...
			const electric_report_file = "./reports/last_7_days_electric.json";
...
const req_g = https.request(options_gas, (res) => {
	...
	res.on('end', () => {
		try {
			...
			//Write out the report to the json file
			const json_str = JSON.stringify(output, null, 2);
			const gas_report_file = "./reports/last_7_days_gas.json";
