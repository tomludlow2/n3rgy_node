...
const req_e = https.request(options_electricity, (res) => {
	...
	res.on('end', () => {
		try {
			...
			fs.writeFileSync('/home/tom/n3rgy/n3rgy_node/electricity.json', JSON.stringify(processedData, null, 2));
...
const req_g = https.request(options_gas, (res) => {
	...
	res.on('end', () => {
		try {
			...
			//Write the processed data to a JSON file
			fs.writeFileSync('/home/tom/n3rgy/n3rgy_node/gas.json', JSON.stringify(processedData, null, 2));
